...
import { Plus, Trash2, Check, AlertTriangle, Eye } from "lucide-react";
...
const Supplies = () => {
  ...
  const addItem = () => {
    ...
    if (!store) return;
    if (newItems.some(i => i.product_id === selProduct)) {
      toast.error("Product already added");
      return;
    }
    const available = getStock(selProduct, store.id);
    if (Number(selQty) > available) {
      toast.error(`Only ${available} cartons in store`);
      return;
    }
  ...
  const handleCreateSupply = async () => {
    if (!store || !toLocationId || newItems.length === 0) {
      toast.error("Select shop and add items");
      return;
    }
    ...
    await supabase.from("transfer_items").insert(
      newItems.map(item => ({
        ...
        sent_cartons: item.sent_cartons,
        received_cartons: 0,
      }))
    );
    ...
    for (const item of newItems) {
      ...
      await supabase.from("stock").upsert({
        ...
        cartons: Math.max(0, current - item.sent_cartons),
  ...
  const openTransfer = (t: Transfer) => {
    ...
    setResponseItems(t.items.map(i => ({
      id: i.id,
      received_cartons: i.received_cartons || i.sent_cartons,
      issue_note: i.issue_note || "",
    })));
    setViewOpen(true);
  };
    ...
    if (allMatch) {
      ...
      for (const item of selectedTransfer.items) {
        ...
        await supabase.from("stock").upsert({
          ...
          cartons: current + item.sent_cartons,
        }, { onConflict: "product_id,location_id" });
      }
      ...
      for (const ri of responseItems) {
        await supabase.from("transfer_items").update({
          received_cartons: ri.received_cartons,
          issue_note: ri.issue_note || null,
        }).eq("id", ri.id);
      }
  ...
  // Resolve dispute (admin/store staff)
  const handleResolve = async () => {
    ...
    // Use received amounts for stock
    for (const ri of responseItems) {
      ...
      await supabase.from("stock").upsert({
        product_id: item.product_id,
        location_id: selectedTransfer.to_location_id,
        cartons: current + ri.received_cartons,
      }, { onConflict: "product_id,location_id" });

      // Return difference to store
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="space-y-4 pt-2">
                        ...
                        <SelectContent>
                          {activeProducts.map(p => (
                            <SelectItem key={p.id} value={p.id}>
                              {p.name} ({getStock(p.id, store.id)} in store)
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                        ...
                        {newItems.map((item, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center justify-between text-sm bg-muted rounded-md px-3 py-2">
                              ...
                              <button onClick={() => setNewItems(prev => prev.filter((_, i) => i !== idx))} className="text-destructive hover:text-destructive/80">
                ...
                <Button className="w-full" onClick={handleCreateSupply} disabled={!toLocationId || newItems.length === 0}>
                  Create Supply
                </Button>
              </div>
            </DialogContent>
              ...
              return (
                <tr key={t.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize ${statusColor(t.status)}`}>
                      {t.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <Button variant="ghost" size="sm" onClick={() => openTransfer(t)}>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              Supply Details
              {selectedTransfer && (
                <span className={`ml-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize ${statusColor(selectedTransfer.status)}`}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-foreground">{locations.find(l => l.id === selectedTransfer.from_location_id)?.name}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">To</p>
                  <p className="font-medium text-foreground">{locations.find(l => l.id === selectedTransfer.to_location_id)?.name}</p>
                </div>
              </div>
                    ...
                    <div key={item.id} className="rounded-lg border border-border p-3 space-y-2">
                      ...
                      {selectedTransfer.status === "pending" && role === "shop_staff" && (
                        <div className="space-y-1">
                          ...
                          <Input
                            type="number"
                            value={ri?.received_cartons ?? item.sent_cartons}
                            onChange={e => {
                              const val = Number(e.target.value);
                              setResponseItems(prev => prev.map((r, i) => i === idx ? { ...r, received_cartons: val } : r));
                            }}
                            className="h-8"
                          />
                          {ri && ri.received_cartons !== item.sent_cartons && (
                            <Textarea
                              ...
                              value={ri.issue_note}
                              onChange={e => setResponseItems(prev => prev.map((r, i) => i === idx ? { ...r, issue_note: e.target.value } : r))}
                              className="mt-1"
                              rows={2}
                            />
                          )}
                        ...
                        <div className="text-sm space-y-1">
                          ...
                          {item.issue_note && (
                            <p className="text-destructive text-xs flex items-center gap-1">
                              <AlertTriangle className="h-3 w-3" /> {item.issue_note}
                            </p>
                          )}
                          {canResolve && (
                            <div className="space-y-1">
                              ...
                              <Input
                                type="number"
                                value={ri?.received_cartons ?? item.received_cartons}
                                onChange={e => {
                                  const val = Number(e.target.value);
                                  setResponseItems(prev => prev.map((r, i) => i === idx ? { ...r, received_cartons: val } : r));
                                }}
                      ...
                      {selectedTransfer.status === "accepted" && (
                        <p className="text-sm text-success flex items-center gap-1">
                          <Check className="h-3 w-3" /> Received: {item.received_cartons || item.sent_cartons} ctns
                        </p>
                      )}
                    </div>
                  );
