...
const ShopSales = () => {
  ...
  const receiptSale = receiptSaleId ? mySales.find(s => s.id === receiptSaleId) : null;
  ...
  return (
    <div>
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="stat-card"><p className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Cart Items</p><p className="text-xl font-bold text-primary">{cartCount}</p></div>
        <div className="stat-card"><p className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Cart Total</p><p className="text-xl font-bold text-warning">{fmt(cartTotal)}</p></div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <div className="lg:col-span-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {filteredProducts.map(p => {
              ...
              return (
                <button key={p.id} onClick={() => addToCart(p)} disabled={avail <= 0}
                  className={`group relative rounded-xl border p-3 text-left transition-all ${avail <= 0 ? "border-border bg-muted/50 opacity-50 cursor-not-allowed" : inCart ? "border-primary bg-primary/5 shadow-sm" : "border-border bg-card hover:border-primary/50 hover:shadow-sm"}`}>
                  <p className="text-sm font-semibold text-foreground truncate">{p.name}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{fmt(p.price_per_carton)}/ctn</p>
                  <div className="mt-2 flex items-center justify-between">
                    <span className={`text-[11px] font-medium ${avail === 0 ? "text-destructive" : avail < 5 ? "text-warning" : "text-success"}`}>{avail} avail</span>
                    {inCart && <span className="rounded-full bg-primary px-2 py-0.5 text-[10px] font-bold text-primary-foreground">{inCart.cartons} in cart</span>}
                  </div>
                </button>
              );
            })}
        ...
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-border bg-card p-4 sticky top-4">
            <div className="flex items-center gap-2 mb-4"><ShoppingCart className="h-5 w-5 text-primary" /><h2 className="font-display text-base font-semibold text-foreground">Cart</h2></div>
            <div className="mb-3"><Input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Customer name" className="text-sm" /></div>
            {cart.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                {cart.map(item => (
                  <div key={item.product_id} className="flex items-center gap-2 rounded-lg bg-muted/50 p-2">
                    ...
                    <button onClick={() => setCart(prev => prev.filter(c => c.product_id !== item.product_id))} className="text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                ))}
              </div>
            )}
            {cart.length > 0 && (
              <div className="mt-4 space-y-3">
                ...
                <Button className="w-full" size="lg" onClick={handleCheckout}>Complete Sale — {fmt(cartTotal)}</Button>
              </div>
            )}
          </div>
        </div>
              ...
              {filteredSales.map(sale => (
                <tr key={sale.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => { setReceiptSaleId(sale.id); setReceiptOpen(true); }} className="text-primary hover:text-primary/80">
      ...
      {receiptSale && (
        <ReceiptDialog
          ...
          type="sale"
          receiptNumber={receiptSale.id.slice(-6).toUpperCase()}
          ...
          total={receiptSale.total_amount}
        />
      )}
    </div>
  );
